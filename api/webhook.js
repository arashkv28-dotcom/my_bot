export default async function handler(req, res) {
  ...
  // ----------------------------------------------------
  // بخش 1: قابلیت نستعلیق (اولویت اول)
  // ----------------------------------------------------
  if (message.text && message.text.startsWith('نستعلیق ')) {
    ...
    // ساخت عکس با سرویس مطمئن (بدون نیاز به API خارجی)
    // از سرویس رایگان quickchart.io استفاده می‌کنیم
    const encodedText = encodeURIComponent(persianText);
    const imageUrl = `https://quickchart.io/wordcloud?text=${encodedText}&width=800&height=400&backgroundColor=%231a1a2e&fontFamily=Arial&fontWeight=bold&colors=%23e0e0ff,%23b0b0ff,%238080ff`;

    try {
      await tgApi('sendPhoto', {
        chat_id: chatId,
        photo: imageUrl,
        caption: `✍️ ${persianText}`
      });
    } catch (e) {
      // اگر عکس کار نکرد، متن زیبا بفرست
      await tgApi('sendMessage', {
        chat_id: chatId,
        text: `✍️ *${persianText}*`,
        parse_mode: 'Markdown'
  ...
  // ----------------------------------------------------
  // بخش 2: فیلتر پیام‌ها (فقط برای اعضای معمولی)
  // ----------------------------------------------------
