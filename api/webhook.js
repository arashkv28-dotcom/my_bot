export default async function handler(req, res) {
  ...
  const sendAutoDeleteMessage = async (chatId, text, seconds = 5) => {
    ...
    if (sent && sent.result) {
      ...
      await tgApi('deleteMessage', { chat_id: chatId, message_id: warnId });
    }
  };
  ...
  // ----------------------------------------------------
  // بخش 2: هوش مصنوعی Gemini (با کلمه "رباتی")
  // ----------------------------------------------------
  if (message.text && message.text.startsWith('رباتی')) {
    const userQuestion = message.text.replace('رباتی', '').trim();
    ...
    try {
      ...
      await tgApi('sendMessage', {
        chat_id: chatId,
        reply_to_message_id: messageId,
        text: '❌ متأسفم، در حال حاضر نمی‌توانم پاسخ دهم. لطفاً دوباره امتحان کنید.'
      });
    }
